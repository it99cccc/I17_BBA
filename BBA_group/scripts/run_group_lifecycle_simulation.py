...
class GroupLifecycleSimulator:
    ...
    def _log_detailed_yearly_results(
        ...
        for policy_state in self.policy_states:
            ...
            # 修正：保险合同收入合计 = -(赔付净额) - (RA净额) + CSM摊销 + IACF摊销 + 经验调整
            # 赔付净额 = revenue_claims_gross - revenue_claims_lc_alloc
            # RA净额 = revenue_ra_gross - revenue_ra_lc_alloc
            # CSM摊销和IACF摊销本身为负值
            revenue_total = (
                -(revenue_claims_gross - revenue_claims_lc_alloc) +
                -(revenue_ra_gross - revenue_ra_lc_alloc) +
                revenue_csm_amort +
                revenue_iacf_amort +
                revenue_exp_adj +
                revenue_investment
    ...
    def _run_yearly_measurement_for_policy(
        ...
        # 关键修复：传递批减单标记，确保后续逻辑能正确识别LC符号
        if init_context:
            context.is_reversal_policy = getattr(init_context, 'is_reversal_policy', False)
        else:
            context.is_reversal_policy = getattr(policy_state, 'is_reversal_policy', False)
