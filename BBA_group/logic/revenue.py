...
import pandas as pd
from BBA_group.data_access.loader import get_rates
...
def run(context, logger):
    ...
    from BBA_group.utils.pv_field_desc import describe_field, format_pv_field_in_formula
    ...
    # 注意：亏损分摊应从收入中扣除
    # 修正：预期赔付/RA释放（正值）应取反作为收入（负值）
    context.total_revenue = (
        -(revenue_claims_expenses_gross - revenue_claims_expenses_lc_alloc) +  # 赔付与费用（净额，取反）
        -(ra_release_gross - ra_release_lc_alloc) +  # RA释放（净额，取反）
        context.revenue_csm_amort +  # CSM摊销（负值）
        context.revenue_iacf_amort +  # IACF摊销（负值）
    ...
    logger.log_item(
        ...
        context.total_revenue,
        note=f"合计 = -({revenue_claims_expenses_gross} - {revenue_claims_expenses_lc_alloc}) + -({ra_release_gross} - {ra_release_lc_alloc}) + {context.revenue_csm_amort} + {context.revenue_iacf_amort} + {context.revenue_exp_adj} = {context.total_revenue}"
    )